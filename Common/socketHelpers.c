...
void close_socket(int fdL) {
    if (TEMP_FAILURE_RETRY(close(fdL)) < 0)ERR("close"); //fdL - file descriptor to socket
}
//end functions
//SERVER functions
...
int64_t bulk_read(int fd, char *buf, size_t count){
    ...
    size_t len=0;
    do{
        c= (int) TEMP_FAILURE_RETRY(read(fd, buf, count));
        if(c<0) return c;
        if(c==0) return (int64_t) len; //EOF
        buf+=c;
        len+=c;
        count-=c;
    }while(count>0);
...
int64_t bulk_write(int fd, char *buf, size_t count){
    ...
    size_t len=0;
    do{
        c= (int) TEMP_FAILURE_RETRY(write(fd, buf, count));
        if(c<0) return c;
        buf+=c;
        len+=c;
        count-=c;
...
int bind_inet_socket(uint16_t port,int type){ //type: SOCK_STREAM (tcp), SOCK_DGRAM (udp)
